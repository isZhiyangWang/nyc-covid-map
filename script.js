...
async function makeMap(url, date_input) {
  ...
  const top_cases = zipcode_cases.sort((a, b) => b.totals[METRIC] - a.totals[METRIC]);
  topCasesAll = zipcode_cases.sort((a, b) => b.totals[METRIC] - a.totals[METRIC]);
  let path = d3.geoPath();
  ...
  let projection = d3
    .geoMercator()
    .scale(140000) //.center(center).translate([width/2,height/2])
  ...
  callout.select('h2').text(() => 'Hover, drag, and zoom on the map to interact');
  d3.select('#callout_zipcode')
    .insert('svg', '#callout_zipcode_text')
    .attr('class', 'color_rect_svg')
    .attr('id', 'callout_zipcode_svg') //.attr('width', '28px').attr('height', '28px')
    .append('rect')
  ...
  let top_ranks = callout
    ...
    .append('li')
    .on('mouseover', () => {
      callout.select('h2').text(() => 'Click to locate on the map');
      callout.select('h3').text(() => '');
    ...
    .on('click', (e) => {
      ...
      d3.selectAll('.zipcode_path').style('stroke', STROKE_LIGHT).style('stroke-width', 1);
      d3.select(`#path_${zipcode}`).style('stroke', STROKE_DARK).style('stroke-width', 3);
  ...
  top_ranks
    .append('svg')
    .attr('class', 'color_rect_svg')
    .attr('id', 'rank_rect') //.attr('width', '18px').attr('height', '18px')
    .append('rect')
  ...
  svg.on('mouseleave', () => {
    ...
    callout.select('h3').text(() => '');
    d3.select('#callout_zipcode_svg rect').attr('fill', 'rgba(0, 0, 0, 0)');
    d3.select(`#path_${last_hover_zipcode}`).style('stroke', STROKE_LIGHT).style('stroke-width', 1);
  });
